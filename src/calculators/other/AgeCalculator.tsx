...
export default function AgeCalculator() {
    ...
    const calculateAge = () => {
        ...
        if (days < 0) {
            months--;
            // Get days in previous month relative to target
        ...
        addHistory(
            ...
            `Born: ${birthDate}, Age at: ${targetDate}`
        );
    };

    const handleHistorySelect = (item: any) => {
        setBirthDate(item.inputs.birthDate);
        setTargetDate(item.inputs.targetDate || todayStr);
    };
            ...
            <div className="max-w-2xl mx-auto bg-white dark:bg-slate-900 p-8 rounded-3xl shadow-sm border border-slate-100 dark:border-slate-800">
                <div className="space-y-6">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div className="space-y-2">
                            <label className="block text-sm font-medium text-slate-700 dark:text-slate-300">Age at Date</label>
                            <div className="relative">
                                <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                                    <Calendar className="h-5 w-5 text-slate-400" />
                                </div>
                    ...
                    {age && (
                        <div className="mt-8 p-6 bg-blue-50 dark:bg-blue-900/20 rounded-2xl border border-blue-100 dark:border-blue-900/30 text-center">
                            <h3 className="text-sm font-semibold text-blue-800 dark:text-blue-300 uppercase tracking-wide mb-4">Age Result</h3>
