            ...
            <div className="bg-white dark:bg-slate-900 p-6 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-800 space-y-6">
                <div className="grid grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="number" value={original} onChange={(e) => setOriginal(Number(e.target.value))} className="block w-full px-4 py-2 border border-slate-200 dark:border-slate-700 rounded-lg focus:ring-2 focus:ring-blue-500 text-lg bg-white dark:bg-slate-800 text-slate-900 dark:text-white" />
                    </div>
                    <div>
                        <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">New Value</label>
                        <input type="number" value={newValue} onChange={(e) => setNewValue(Number(e.target.value))} className="block w-full px-4 py-2 border border-slate-200 dark:border-slate-700 rounded-lg focus:ring-2 focus:ring-blue-500 text-lg bg-white dark:bg-slate-800 text-slate-900 dark:text-white" />
                    </div>
                </div>
                ...
                <div className={`p-8 rounded-2xl text-center border transition-colors ${result?.type === 'increase' ? 'bg-green-50 dark:bg-green-900/20 border-green-100 dark:border-green-900/30' : result?.type === 'decrease' ? 'bg-red-50 dark:bg-red-900/20 border-red-100 dark:border-red-900/30' : 'bg-slate-50 dark:bg-slate-800 border-slate-200 dark:border-slate-700'}`}>
                    {original === 0 ? (
                        ...
                        <>
                            <div className="flex items-center justify-center gap-2 mb-2">
                                ...
                                <span className={`text-sm font-bold uppercase tracking-wide ${result?.type === 'increase' ? 'text-green-700 dark:text-green-400' : result?.type === 'decrease' ? 'text-red-700 dark:text-red-400' : 'text-slate-500 dark:text-slate-400'}`}>
                                    {result?.type}
                                </span>
                            </div>
                            ...
                            <div className="grid grid-cols-2 gap-4 text-sm max-w-sm mx-auto">
                                <div className="bg-white/50 dark:bg-slate-900/50 p-3 rounded-lg"><p className="text-slate-500 dark:text-slate-400 mb-1">Difference</p><p className="font-semibold text-slate-900 dark:text-white">{result?.difference && result.difference > 0 ? '+' : ''}{result?.difference.toLocaleString()}</p></div>
                                <div className="bg-white/50 dark:bg-slate-900/50 p-3 rounded-lg"><p className="text-slate-500 dark:text-slate-400 mb-1">% of Original</p><p className="font-semibold text-slate-900 dark:text-white">{result?.percentOfOriginal.toLocaleString(undefined, { maximumFractionDigits: 2 })}%</p></div>
                            </div>
                        </>
                    )}
                </div>
